...
const emit = defineEmits(['add-to-cart', 'remove', 'notify-me']);
...
const categoryLabel = computed(
    () => product.value.categories?.[0]?.name ?? 'Pastry',
);

const addedLabel = computed(() => {
    const label = formatFavouritedAgo(
        props.favourite.favourited_at ?? props.favourite.created_at,
    );
    return label.replace(/^Added /i, 'Saved ');
});

const ratingDisplay = computed(() => {
    const rating = Number(product.value.average_rating ?? 0);
        ...
        <div class="flex items-center gap-lg sm:justify-end">
            ...
            <div class="flex gap-sm">
                <button
                    v-if="isOutOfStock"
                    type="button"
                    class="flex h-10 w-10 items-center justify-center rounded-full border border-outline-variant text-on-surface-variant transition-colors hover:bg-surface-container"
